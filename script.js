...
class TaskManagement extends AuxiliaryFunctions {
	constructor() {
...
function exampleTypeData(inputData) {
	let result = true;
	inputData.forEach(data => {
		typeof data.id !== "number" || typeof data.text !== "string" ? result = false : null;
	});
	return result;
}
...
// Проверка на корректные типы данных перед загрузкой страницы
exampleTypeData(data) ? loadOriginTask() : console.log("Некорректные данные");
